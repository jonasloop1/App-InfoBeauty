...
public class TelaCadastroEmpresa extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        acao = getIntent().getExtras().getInt("acao");// -1 (inclusão) ou 0 (exclusão ou alteração)
        id = getIntent().getExtras().getLong("id");

        if (acao == -1) {
            // inclusão
            setTitle("Inserir Empresa");
            proximoEmpresaCadastro.setText("Incluir");
            nomeEmpresa.setText("Nome Empresa");
            emailEmpresa.setText("Email Empresa");
            cnpjEmpresa.setText("Cnpj Empresa");
            senhaEmpresa.setText("Senha Empresa");
            confirmasenhaEmpresa.setText("ConfirmaSenha Empresa");
            ...
            aux = dao.buscar(id);
            // preenche a tela com os dados da tabela
            nomeEmpresa.setText("Nome Empresa");
            emailEmpresa.setText("Email Empresa");
            cnpjEmpresa.setText("Cnpj Empresa");
            senhaEmpresa.setText("Senha Empresa");
    ...
    public void alterarInserir(View v) {
        ...
        if (acao == -1) { // inserção
            ...
            dao.alterar(id, nome, email, cnpj,senha,confirmasenha);
        }
        dao.close();
        finish();
    }
